...
import { PRD_TEMPLATE_VARIANTS, PrdTemplateVariant } from '@/lib/documents/prd-templates'

interface PrdTemplateSelectorModalProps {
  isOpen: boolean
...
export function PrdTemplateSelectorModal({
  isOpen,
  currentTemplateId,
  onClose,
  onSelectTemplate,
}: PrdTemplateSelectorModalProps) {
  const [selectedCategory, setSelectedCategory] = useState<string>('all')

  if (!isOpen) return null

  const variants = Object.values(PRD_TEMPLATE_VARIANTS)
      ...
      <div className="relative w-full max-w-5xl max-h-[90vh] bg-app-surface border border-app-border rounded-3xl shadow-2xl flex flex-col overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-xl font-black text-app-fg tracking-tight">PRD Template Library</h2>
              ...
              <p className="text-xs text-app-muted mt-0.5">
                Select a PRD format tailored by company style, organizational maturity, and product type.
              </p>
            </div>
          </div>
        ...
        <div className="px-6 py-3 border-b border-app-border bg-app-bg/50 flex items-center gap-2 overflow-x-auto">
          {[
            ...
            { id: 'startup', label: 'Startup One-Pager' },
            { id: 'leadership', label: 'Executive & PR-FAQ' },
            { id: 'technical', label: 'Technical & Eng Specs' },
            { id: 'enterprise', label: 'B2B Enterprise' },
            { id: 'growth', label: 'Growth & A/B Experiments' },
        ...
        <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredVariants.map((variant) => {
            const IconComp = ICON_MAP[variant.iconName] || Sparkles
