...
import { CompetitorStrategyItem } from '../constants/types'
...
interface CompetitorStrategyMatrixProps {
  strategies: CompetitorStrategyItem[]
  competitorAName: string
  competitorBName: string
  hasEditAccess: boolean
  onUpdate: (updated: CompetitorStrategyItem[]) => void
  setIsDirty: (dirty: boolean) => void
}

export function CompetitorStrategyMatrix({
  strategies,
  competitorAName,
  competitorBName,
  hasEditAccess,
  onUpdate,
  setIsDirty,
}: CompetitorStrategyMatrixProps) {
  const [modalItem, setModalItem] = useState<CompetitorStrategyItem | null>(null)
  ...
  const handleOpenModal = (item?: CompetitorStrategyItem) => {
    if (item) {
      setModalItem(item)
  ...
  const handleSaveModal = (e: React.FormEvent) => {
    ...
    const newItem: CompetitorStrategyItem = {
      id: modalItem?.id || `s_${Date.now()}`,
      dimensionName,
      ourProduct,
      competitorA,
      competitorB,
    ...
    if (modalItem) {
      onUpdate(strategies.map(s => s.id === newItem.id ? newItem : s))
    } else {
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-4">
        <div>
          <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
            Market Strategy & UVP Matrix
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Compare target ICPs, go-to-market motions, and overarching marketing claims.
          </p>
        </div>
        ...
        <table className="w-full text-left text-xs">
          <thead>
            <tr className="border-b border-slate-200 dark:border-slate-800 text-slate-400 font-bold uppercase tracking-wider">
              <th className="py-3 px-4 min-w-[180px]">Strategy Dimension</th>
            ...
            <div className="p-4 sm:p-5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold text-sm text-slate-900 dark:text-white">
                  {modalItem ? 'Edit Strategy Dimension' : 'Add Strategy Dimension'}
                </h3>
                <p className="text-[10px] text-slate-500">Configure market strategy comparison</p>
              </div>
            </div>
            ...
            <form onSubmit={handleSaveModal} className="p-4 sm:p-5 space-y-4">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  placeholder="e.g. Target ICP, Primary UVP"
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  placeholder="e.g. Enterprise focus"
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500"
                />
              </div>
