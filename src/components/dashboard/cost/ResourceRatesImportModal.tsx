...
import { bulkImportResourceRates } from '@/lib/cost/actions'
...
export function ResourceRatesImportModal({ projectId, projectCurrency, onClose, onSuccess }: Props) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const idxName = headers.findIndex(h => h.includes('name') || h === 'resource')
      const idxType = headers.findIndex(h => h.includes('type') || h === 'category')
      const idxRate = headers.findIndex(h => h.includes('rate') || h === 'cost' || h === 'price')
      const idxUnit = headers.findIndex(h => h.includes('unit') || h === 'per')

      if (idxName < 0) {
        throw new Error('CSV must include a "Name" or "Resource" column.')
      }
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        // Parse Rate
        let rate = 0
        if (idxRate >= 0 && cols[idxRate]) {
          // Remove currency symbols, commas, and parse as float
          const sanitizedRate = cols[idxRate].replace(/[^0-9.-]+/g, '')
          rate = parseFloat(sanitizedRate) || 0
        }

        // Parse Type
        let type: ResourceType = 'labor'
        if (idxType >= 0 && cols[idxType]) {
          const rawType = cols[idxType].toLowerCase()
          if (rawType.includes('mat')) type = 'material'
          else if (rawType.includes('fix') || rawType.includes('equip')) type = 'fixed'
        }

        // Parse Unit
        let unit: ResourceUnit = type === 'labor' ? 'hr' : 'flat'
        if (idxUnit >= 0 && cols[idxUnit]) {
          const rawUnit = cols[idxUnit].toLowerCase()
          if (rawUnit === 'hr' || rawUnit === 'hour') unit = 'hr'
          else if (rawUnit === 'day') unit = 'day'
          else if (rawUnit === 'unit') unit = 'unit'
          else if (rawUnit === 'flat' || rawUnit === 'fixed') unit = 'flat'
        }
      ...
      if (toImport.length > 0) {
        const res = await bulkImportResourceRates(projectId, toImport)
      } else {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in">
      <div className="bg-app-surface w-full max-w-2xl rounded-2xl shadow-xl overflow-hidden animate-in zoom-in-95">
        <div className="flex items-center justify-between p-6 border-b border-app-border">
          <h3 className="text-xl font-bold text-app-fg">Import Resource Rates (CSV)</h3>
          ...
          {importMode === 'paste' && (
            <div className="mb-6">
              <p className="text-sm text-app-subtle mb-2">
                Ensure the first row has headers. Example: <strong>Name, Type, Rate, Unit</strong>.
              </p>
              <textarea
                value={csvText}
