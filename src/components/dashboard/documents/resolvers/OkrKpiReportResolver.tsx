  ...
  if (loading) {
    return (
      <div className="flex items-center py-6 px-4 text-slate-400 text-xs bg-slate-50 dark:bg-slate-900/40 rounded-xl border border-slate-200 dark:border-slate-800">
        <Loader2 className="w-4 h-4 animate-spin mr-2.5 text-violet-500" />
  ...
  if (!data || (!data.northStar && data.growthLevers.length === 0 && data.objectives.length === 0)) {
    return (
      <div className="p-8 text-center rounded-2xl bg-slate-50 dark:bg-slate-900/50 border border-dashed border-slate-300 dark:border-slate-800 text-xs text-slate-500 my-4">
        <Compass className="w-8 h-8 text-violet-500 mx-auto mb-2 opacity-80" />
      ...
      <div className="space-y-6 my-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden">
            <span className="text-[10px] font-extrabold text-violet-600 dark:text-violet-400 uppercase tracking-wider block mb-1">PRIMARY NORTH STAR</span>
          ...
          <div className="p-5 rounded-2xl bg-violet-500/10 border border-violet-500/20 text-slate-900 dark:text-white shadow-xs">
            <span className="text-[10px] font-extrabold text-violet-600 dark:text-violet-400 uppercase tracking-wider block mb-1">SUPPORTING LEVERS</span>
            <div className="text-2xl font-extrabold text-violet-600 dark:text-violet-400 mb-1">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <button
                  ...
                  className="p-1 text-slate-400 hover:text-violet-500 rounded transition-colors ml-auto"
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse text-xs">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-slate-800 dark:text-slate-200">
                {data.northStar && (
                  <tr className="bg-violet-50/40 dark:bg-violet-950/20 font-semibold">
                    <td className="p-3.5 pl-5 flex items-center gap-2">
                      <span className="w-2 h-2 rounded-full bg-violet-500" />
                      <strong className="text-violet-600 dark:text-violet-400">{data.northStar.name}</strong>
                    </td>
                    <td className="p-3.5 text-[11px] font-extrabold text-violet-500">⭐ North Star</td>
                    ...
                    <td className="p-3.5 pr-5 text-right">
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-extrabold uppercase bg-violet-500/10 text-violet-500 border border-violet-500/20">
              ...
              <div className="p-4 bg-slate-50 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[10px] font-extrabold px-2 py-0.5 rounded-md bg-violet-500/10 text-violet-500 border border-violet-500/20 uppercase tracking-wider">
                      OBJECTIVE
                    </span>
                ...
                <div className="text-right font-extrabold text-violet-600 dark:text-violet-400 text-sm">
                  {obj.progress || 0}%
                </div>
              </div>
                    ...
                    {krs.map((kr) => (
                      <div key={kr.id} className="p-3 rounded-xl bg-slate-50/60 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-800/80 flex items-center justify-between text-xs">
                        <div>
                          ...
                          <span className="text-[11px] text-slate-500">Baseline: {kr.baseline_value} ➔ Target: <strong className="text-violet-600 dark:text-violet-400">{kr.target_value}</strong></span>
                        </div>
                        <div className="flex items-center gap-3">
                          <span className="text-violet-600 dark:text-violet-400 font-extrabold text-sm">{kr.current_value} ({kr.progress || 0}%)</span>
