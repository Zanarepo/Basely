...
export default function ScopeToRiceAutomationBanner({
  projectId,
  organizationId,
  freeText,
  onShowToast,
  isSnapshot = false,
}: ScopeToRiceAutomationBannerProps) {
  const {
    isGenerating,
    generatedItems,
    isModalOpen,
          ...
          <div className="flex items-center gap-2">
            {generatedItems.length > 0 && (
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-bold border border-emerald-500/20 text-xs hover:bg-emerald-500/20 transition-all cursor-pointer shadow-sm"
              >
            ...
            >
              {isGenerating ? (
                ...
                <>
                  ...
                  <span>Convert Scope to RICE Backlog</span>
                </>
              )}
            </button>
          </div>
            ...
            <div className="space-y-0.5 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <h3 className="text-xs font-bold text-slate-900 dark:text-slate-100 uppercase tracking-wider">
                  Scope → RICE Backlog Automation Engine
                </h3>
              ...
              <p className="text-xs text-slate-600 dark:text-slate-300">
                Extract major deliverables and phases from the Scope Statement, estimate RICE parameters, and insert them into the product backlog for execution.
              </p>
            </div>
          </div>
        </div>
