  ...
  const handleGenerate = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (res.ok && res.data) {
        ...
        onShowToast?.('error', res.error || 'Failed to generate synthesis')
      }
    } catch (err: any) {
      console.error('AiClosureSynthesisBanner generation error:', err)
      onShowToast?.('error', 'AI generation is temporarily unavailable. Please try again shortly.')
    } finally {
      setIsGenerating(false)
    }
  }

  const title = docType === 'lessons_learned' 
    ? 'LESSONS LEARNED SYNTHESIS ENGINE' 
    : 'POST-IMPLEMENTATION REVIEW ENGINE'
    
  const description = docType === 'lessons_learned'
    ? 'Praz-AI will automatically scan your WBS completions, Issues, and Risks to determine what worked well, what failed, and generate recommendations for the future.'
    : 'Praz-AI will automatically evaluate your original project objectives against the final actuals to synthesize a comprehensive outcome assessment and ROI realization.'
