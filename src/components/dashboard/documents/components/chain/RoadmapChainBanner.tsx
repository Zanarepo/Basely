...
import { synthesizePrdFromRoadmap } from '@/lib/documents/ai-chain-actions'

interface RoadmapChainBannerProps {
  projectId: string
  freeText: Record<string, string>
  onShowToast: (type: 'success' | 'error', msg: string) => void
}

export default function RoadmapChainBanner({
  projectId,
  freeText,
  onShowToast,
}: RoadmapChainBannerProps) {
  ...
  const handleSynthesize = async () => {
    ...
    try {
      const res = await synthesizePrdFromRoadmap(projectId, freeText)
      if (res.ok) {
        setIsDone(true)
        onShowToast(
          'success',
          '⚡ PRD synthesized! Switch to Product Requirements Document (PRD) to inspect User Stories & Acceptance Criteria.'
        )
      } else {
        onShowToast('error', res.error || 'Failed to synthesize PRD')
      }
    } catch (err: any) {
      console.error(err)
        ...
        <div className="flex items-center gap-3.5 min-w-0">
          ...
          <div className="space-y-0.5 min-w-0">
            ...
            <p className="text-xs text-slate-600 dark:text-slate-300">
              Convert Roadmap initiatives, strategic themes, and target outcomes into a formal Product Requirements Document (PRD) automatically.
            </p>
          </div>
        </div>
          ...
          >
            {isSynthesizing ? (
              <>
                ...
                <span>Praz-AI Generating PRD...</span>
              </>
            ) : (
              <>
                <Sparkles className="w-4 h-4 text-violet-200" />
                <span>{isDone ? 'Re-generate' : 'Generate PRD Specs'}</span>
