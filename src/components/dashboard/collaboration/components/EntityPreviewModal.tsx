...
export function EntityPreviewModal({ type, id, projectId, onClose }: EntityPreviewModalProps) {
  ...
  useEffect(() => {
    ...
    const staticPages = ['stakeholders', 'raci', 'charter', 'gantt', 'cost_estimation', 'cost_resources', 'cost_timephasing', 'cost_baselines', 'cost_actuals', 'wbs_board', 'wbs_grid', 'wbs_unassigned', 'product_doc']
  ...
  const navigateToDashboard = () => {
    ...
    if (type === 'product_doc') url = `/dashboard/projects/${projectId}?tab=documents&doc=${id}`
  ...
  const getHeaderIcon = () => {
    ...
    if (type.startsWith('cost_') || type === 'gantt' || type === 'product_doc') return <FileText className="w-4 h-4 text-blue-500" />
    if (type === 'wbs_unassigned') return <CheckSquare className="w-4 h-4 text-amber-500" />
    return <CheckSquare className="w-4 h-4 text-violet-500" />
  }
      ...
      <div className="w-full max-w-md bg-app-surface border border-app-border rounded-xl shadow-2xl flex flex-col overflow-hidden animate-fade-in">
        ...
        <div className="p-4 border-t border-app-border bg-app-surface-solid flex justify-end">
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 bg-violet-500 hover:bg-violet-600 disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm font-semibold rounded-lg transition-colors"
          >
            {isNavigating ? (
              <>
