  ...
  return (
    <div className="bg-app-surface border border-app-border rounded-xl shadow-sm overflow-hidden flex flex-col h-full">
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input 
              ...
              className="pl-9 pr-4 py-2 bg-app-surface border border-app-border rounded-lg text-sm focus:outline-none focus:border-violet-500 w-64"
            />
          </div>
        </div>
      </div>
      ...
      {selectedUserIds.size > 0 && editingUserId !== 'bulk' && (
        <div className="bg-violet-500/10 border-b border-violet-500/20 px-6 py-3 flex items-center justify-between">
          <span className="text-sm font-medium text-violet-500">{selectedUserIds.size} member{selectedUserIds.size > 1 ? 's' : ''} selected</span>
          <button onClick={startBulkEdit} className="text-sm bg-violet-500 text-white px-4 py-1.5 rounded-lg hover:bg-violet-600 font-medium shadow-sm transition-colors">
            Bulk Assign & Edit
          </button>
        </div>
      )}
        ...
        <table className="w-full text-left border-collapse min-w-[800px]">
            ...
            <tr className="bg-app-muted-surface border-b border-app-border text-xs uppercase tracking-wider text-app-muted">
              <th className="px-6 py-4 w-12 text-center">
                <input 
                  ...
                  className="h-4 w-4 rounded border-app-border text-violet-600 focus:ring-violet-500"
                />
              </th>
          ...
          <tbody className="divide-y divide-app-border text-sm">
            {editingUserId === 'bulk' && editState && (
              <tr className="bg-violet-500/5 hover:bg-violet-500/10 transition-colors border-b-2 border-violet-500/20">
                <td className="px-6 py-4 w-12 text-center">
                  <div className="h-2 w-2 rounded-full bg-violet-500 mx-auto" />
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="font-semibold text-violet-500">Bulk Editing {selectedUserIds.size} Members</div>
                ...
                <td className="px-4 py-4 text-center">
                  <input 
                    ...
                    className="h-4 w-4 rounded border-violet-500/30 text-violet-600 focus:ring-violet-500"
                  />
                </td>
                <td className="px-4 py-4 text-center">
                  <input 
                    ...
                    className="h-4 w-4 rounded border-violet-500/30 text-emerald-600 focus:ring-emerald-500"
                  />
                </td>
                <td className="px-4 py-4 text-center">
                  <input 
                    ...
                    className="h-4 w-4 rounded border-violet-500/30 text-amber-600 focus:ring-amber-500"
                  />
                </td>
                <td className="px-4 py-4 text-center">
                  <input 
                    ...
                    className="h-4 w-4 rounded border-violet-500/30 text-sky-600 focus:ring-sky-500"
                  />
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <div className="flex justify-end gap-2">
                    ...
                    <button onClick={handleBulkSave} disabled={isSaving} className="px-3 py-1.5 bg-violet-500 text-white rounded-md hover:bg-violet-600 transition-colors font-semibold shadow-sm flex items-center gap-1.5">
                ...
                <tr key={member.userId} className={`group hover:bg-app-hover/50 transition-colors ${!member.isAssigned && !isEditing ? 'opacity-60 grayscale' : ''}`}>
                  <td className="px-6 py-4 text-center">
                    <input 
                      ...
                      className={`h-4 w-4 rounded border-app-border text-violet-600 focus:ring-violet-500 transition-opacity ${selectedUserIds.has(member.userId) ? 'opacity-100' : 'opacity-0 group-hover:opacity-100 focus:opacity-100'}`}
                    />
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center gap-3">
                      <div className="h-8 w-8 rounded-full bg-violet-500/10 flex items-center justify-center text-violet-500 font-bold border border-violet-500/20">
                  ...
                  <td className="px-4 py-4 text-center">
                    <input 
                      ...
                      className="h-4 w-4 rounded border-app-border text-violet-600 focus:ring-violet-500 disabled:opacity-50"
                    />
                  </td>
                  <td className="px-4 py-4 text-center">
                    <input 
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    {isEditing ? (
                      <div className="flex justify-end gap-2">
                        ...
                        <button onClick={() => handleSave(member.userId)} disabled={isSaving} className="p-1.5 text-violet-500 hover:text-violet-600 rounded-md hover:bg-violet-500/10 transition-colors">
                          <Save className="h-4 w-4" />
                        </button>
                      </div>
                    ) : (
                      hasEditAccess && editingUserId !== 'bulk' && (
                        <div className="opacity-0 group-hover:opacity-100 transition-opacity flex justify-end">
                          <button 
                            ...
                            className="text-app-muted hover:text-violet-500 transition-colors px-2 py-1 rounded-md hover:bg-app-hover inline-flex items-center gap-1 text-xs font-semibold"
                          >
