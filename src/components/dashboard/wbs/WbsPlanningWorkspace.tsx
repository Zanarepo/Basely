  ...
  const handleMoveNode = (
    ...
    if (position === 'inside') {
      ...
      if (position === 'before') {
        ...
        // Shift following elements up
        elements.forEach((el) => {
          if (el.parentId === newParentId && el.sortOrder >= newSortOrder && el.id !== draggedId) {
            el.sortOrder += 1
          }
        })
      } else {
    ...
    // Apply move changes locally
    const optimisticList = elements.map((item) => {
      if (item.id === draggedId) {
        return { ...item, parentId: newParentId, sortOrder: newSortOrder }
      }
      return item
    })
    ...
    startTransition(async () => {
      const result = await moveWbsElement(draggedId, projectId, newParentId, newSortOrder)
      if (result.ok) {
