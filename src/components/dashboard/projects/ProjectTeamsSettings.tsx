...
import { testTeamsWebhook } from '../../../lib/integrations/teams-actions'

export default function ProjectTeamsSettings({
  projectId,
}: {
  projectId: string
}) {
  ...
  const handleSave = async () => {
    ...
    const { error } = await supabase
      .from('projects')
      .update({ teams_webhook_url: webhookUrl || null })
      .eq('id', projectId)
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-2">
        <label htmlFor={`teams-webhook-url-${projectId}`} className="block text-sm font-medium text-app-fg">
          Microsoft Teams Webhook URL
        </label>
        <input
          id={`teams-webhook-url-${projectId}`}
        ...
        <p className="text-xs text-app-muted">
          You can create a Connector Webhook in your Microsoft Teams Channel settings.
          Leave this empty to disable Teams notifications for this project.
        </p>
      </div>
      ...
      <div className="pt-2 flex flex-col gap-4">
        ...
        <div className="flex items-center gap-2 h-5">
          ...
          {testSuccess === true && <span className="text-sm font-medium text-emerald-500">Test message sent! Check your Teams.</span>}
