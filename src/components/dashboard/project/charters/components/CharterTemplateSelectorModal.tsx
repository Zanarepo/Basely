...
import { CHARTER_TEMPLATE_VARIANTS, CharterTemplateVariant } from '@/lib/documents/charter-templates'

interface CharterTemplateSelectorModalProps {
  isOpen: boolean
...
export function CharterTemplateSelectorModal({
  isOpen,
  currentTemplateId,
  onClose,
  onSelectTemplate,
}: CharterTemplateSelectorModalProps) {
  const [selectedCategory, setSelectedCategory] = useState<string>('all')

  if (!isOpen) return null

  const variants = Object.values(CHARTER_TEMPLATE_VARIANTS)
      ...
      <div className="relative w-full max-w-5xl max-h-[90vh] bg-app-surface border border-app-border rounded-3xl shadow-2xl flex flex-col overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-xl font-black text-app-fg tracking-tight">Project Charter Templates</h2>
                <span className="px-2.5 py-0.5 rounded-full bg-violet-500/15 text-violet-600 dark:text-violet-300 text-[10px] font-extrabold uppercase tracking-wider">
                  {variants.length} Industry Models
                </span>
              </div>
              <p className="text-xs text-app-muted mt-0.5">
                Select a Charter format tailored to your specific project type, industry, and goals.
              </p>
            </div>
          </div>
        ...
        <div className="px-6 py-3 border-b border-app-border bg-app-bg/50 flex items-center gap-2 overflow-x-auto">
          {[
            ...
            { id: 'software', label: 'Agile & Software' },
            { id: 'enterprise', label: 'Enterprise & B2B' },
            { id: 'construction', label: 'Construction & Eng.' },
            { id: 'operations', label: 'Process & Automation' },
            { id: 'marketing', label: 'Marketing & Campaigns' },
            { id: 'transformation', label: 'Business Transformation' },
          ].map((cat) => (
            <button
              key={cat.id}
              type="button"
        ...
        <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredVariants.map((variant) => {
            const IconComp = ICON_MAP[variant.iconName] || FileCheck
