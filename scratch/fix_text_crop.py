...
def fix_text_crop():
    ...
    # Let's crop text precisely: x from 170 to 820 (stopping right after 'r')
    text_crop = clean.crop((170, 410, 820, 545))
    
    # Remove any stray pixels on far right of text_crop
    tc_data = np.array(text_crop)
    # Zero out last 10 pixels on right if they are thin vertical artifact
    tc_data[:, -15:, 3] = 0
    text_crop_clean = Image.fromarray(tc_data, mode='RGBA')
    
    # 1. Stacked Logo
    canvas_w, canvas_h = 700, 480
    stacked = Image.new('RGBA', (canvas_w, canvas_h), (0, 0, 0, 0))
    stacked.paste(icon_crop, (50, 20), icon_crop)
    stacked.paste(text_crop_clean, (50, 305), text_crop_clean)
    ...
    # 2. Horizontal Navbar Logo (with SOLID WHITE TEXT)
    nav_h = 70
    scale_icon = nav_h / icon_crop.height
    icon_nav = icon_crop.resize((int(icon_crop.width * scale_icon), nav_h), Image.Resampling.LANCZOS)
    
    scale_text = (nav_h * 0.65) / text_crop_clean.height
    text_nav = text_crop_clean.resize((int(text_crop_clean.width * scale_text), int(text_crop_clean.height * scale_text)), Image.Resampling.LANCZOS)
    
    nav_w = icon_nav.width + text_nav.width + 20
    navbar = Image.new('RGBA', (nav_w + 30, nav_h + 16), (0, 0, 0, 0))
    navbar.paste(icon_nav, (15, 8), icon_nav)
    
    text_y_pos = 8 + int((nav_h - text_nav.height) / 2) + 2
